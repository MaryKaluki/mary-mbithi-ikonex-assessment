...
const BarChart = ({ value, max = 100, color = 'bg-primary' }) => {
...
const SPCReport = () => {
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await window.axios.get('/api/admin/spc/students');
    ...
    const loadReport = async (student) => {
        ...
        try {
            const res = await window.axios.get(`/api/admin/spc/${student.id}`);
            setReport(res.data);
        } catch { /* silent */ }
        finally { setReportLoading(false); }
    };

    const filtered = gradeFilter ? students.filter(s => s.grade_level === gradeFilter) : students;

    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <h2 className="text-xl md:text-2xl font-bold text-gray-800 dark:text-gray-100">Student Progress Card (SPC)</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Select a student to view their full academic performance</p>
                </div>
                ...
                <div className="lg:col-span-2 space-y-4">
                    {!selected && (
                        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-16 flex flex-col items-center justify-center text-center">
                            ...
                            <p className="text-sm text-gray-400">Select a student from the list to view their progress card.</p>
                        </div>
                    )}
